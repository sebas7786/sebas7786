 ...
 * ============================================================================
 * Instituciones que publican licitaciones (compradoras)
 * CSV: Cédula, Nombre Institucion, Direccion, Telefono, Representante,
...
class ImportadorInstitucionesCompradoras {
    private $conn;
    ...
    public function importar($archivo_csv) {
        try {
            $this->log("🚀 Iniciando importación de instituciones compradoras...");
            ...
            foreach ($rows as $i => $row) {
                ...
                $direccion = $this->limpiar($row[2] ?? '');
                $telefono = $this->limpiar($row[3] ?? '');
                $representante = $this->limpiar($row[4] ?? '');
                $codigo_postal = $this->limpiar($row[5] ?? '');
                $provincia = $this->limpiar($row[6] ?? '');
                $canton = $this->limpiar($row[7] ?? '');
                $distrito = $this->limpiar($row[8] ?? '');
                ...
                // Log cada 500
                if (($i + 1) % 500 == 0) {
                    $this->log("⏳ Procesadas " . ($i + 1) . " instituciones...");
                }
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo'])) {
    $imp = new ImportadorInstitucionesCompradoras($conn);
...
<head>
    ...
    <title>Importar Instituciones Compradoras</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Instituciones que publican licitaciones</p>
        </div>
        ...
        <div class="content">
            ...
            <?php else: ?>
                ...
                <div style="text-align: center;">
                    <a href="importar_instituciones_compradoras.php" class="btn-volver">← Importar otro archivo</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
