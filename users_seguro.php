...
// Habilitar visualización de errores temporalmente (comentar en producción)
...
// ================================
$columnas_necesarias = ['monto_pagar', 'fecha_proximo_pago', 'estado_pago', 'activo', 'notas_pago', 'ultimo_pago', 'monto_ultimo_pago'];
$columnas_existen = true;
$columnas_faltantes = [];

try {
    $stmt = $conn->query("DESCRIBE usuarios");
    $columnas_db = $stmt->fetchAll(PDO::FETCH_COLUMN);

    foreach ($columnas_necesarias as $col) {
        if (!in_array($col, $columnas_db)) {
            $columnas_existen = false;
            $columnas_faltantes[] = $col;
        }
    }
} catch (PDOException $e) {
    die("Error al verificar columnas: " . $e->getMessage());
}

if (!$columnas_existen) {
    echo "<div style='background: #fee2e2; border: 2px solid #ef4444; padding: 2rem; margin: 2rem; border-radius: 8px;'>";
    echo "<h2 style='color: #991b1b;'>⚠️ Error: Columnas Faltantes en la Base de Datos</h2>";
    echo "<p>Faltan las siguientes columnas en la tabla 'usuarios':</p>";
    echo "<ul style='color: #991b1b;'>";
    foreach ($columnas_faltantes as $col) {
        echo "<li><strong>{$col}</strong></li>";
    }
    echo "</ul>";
    echo "<p><strong>SOLUCIÓN:</strong> Ejecuta el archivo <code>sql_usuarios_pagos.sql</code> en tu base de datos MySQL.</p>";
    echo "<pre style='background: #1e293b; color: #fff; padding: 1rem; border-radius: 4px;'>";
    echo "mysql -u tu_usuario -p tu_base_datos &lt; sql_usuarios_pagos.sql";
    echo "</pre>";
    echo "<p>O ejecuta manualmente las siguientes consultas:</p>";
    echo "<pre style='background: #1e293b; color: #fff; padding: 1rem; border-radius: 4px; overflow-x: auto;'>";
    echo "ALTER TABLE usuarios\n";
    echo "ADD COLUMN monto_pagar DECIMAL(10,2) DEFAULT 0,\n";
    echo "ADD COLUMN fecha_proximo_pago DATE NULL,\n";
    echo "ADD COLUMN estado_pago TINYINT(1) DEFAULT 1,\n";
    echo "ADD COLUMN activo TINYINT(1) DEFAULT 1,\n";
    echo "ADD COLUMN notas_pago TEXT NULL,\n";
    echo "ADD COLUMN ultimo_pago DATE NULL,\n";
    echo "ADD COLUMN monto_ultimo_pago DECIMAL(10,2) DEFAULT 0;";
    echo "</pre>";
    echo "</div>";
    exit;
}
...
// ================================
$tablas_necesarias = ['historial_pagos', 'recordatorios_pago'];
$tablas_existen = true;
$tablas_faltantes = [];
...
if (!$tablas_existen) {
    ...
    echo "<h2 style='color: #92400e;'>⚠️ Advertencia: Tablas Faltantes</h2>";
    echo "<p>Faltan las siguientes tablas:</p>";
    echo "<ul style='color: #92400e;'>";
    foreach ($tablas_faltantes as $tabla) {
        echo "<li><strong>{$tabla}</strong></li>";
    }
    echo "</ul>";
    echo "<p><strong>SOLUCIÓN:</strong> Ejecuta el archivo <code>sql_usuarios_pagos.sql</code> para crear las tablas necesarias.</p>";
    echo "<p style='margin-top: 1rem;'><em>El sistema funcionará parcialmente sin estas tablas, pero algunas funciones estarán limitadas.</em></p>";
    echo "</div>";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
                ...
                if ($tablas_existen) {
                    try {
                        $stmt_hist = $conn->prepare("INSERT INTO historial_pagos (usuario_id, monto, fecha_pago, estado, notas, creado_por)
                                                     VALUES (?, ?, NOW(), ?, ?, ?)");
                        $estado_texto = $estado_pago ? 'pagado' : 'pendiente';
                        $stmt_hist->execute([$user_id, $monto_pagar, $estado_texto, $notas_pago, $_SESSION['usuario_id']]);
                    } catch (PDOException $e) {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'registrar_pago') {
            ...
            // Registrar pago solo si la tabla existe
            if (in_array('historial_pagos', $tablas_necesarias) && !in_array('historial_pagos', $tablas_faltantes)) {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'send_reminder') {
            ...
            if ($user) {
                ...
                // Registrar el recordatorio solo si la tabla existe
                if (!in_array('recordatorios_pago', $tablas_faltantes)) {
                    try {
...
// Construir SQL con verificación de tablas
$historial_join = "";
$total_pagado_sql = "0";
$total_pagos_sql = "0";

if (!in_array('historial_pagos', $tablas_faltantes)) {
    $total_pagos_sql = "(SELECT COUNT(*) FROM historial_pagos WHERE usuario_id = u.id)";
    $total_pagado_sql = "(SELECT SUM(monto) FROM historial_pagos WHERE usuario_id = u.id AND estado = 'pagado')";
}

$sql = "SELECT u.*,
        {$total_pagos_sql} as total_pagos,
...
try {
    ...
    // Si falla alguna estadística, continuar con valores por defecto
}
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($usuarios as $user): ?>
                                <?php
                                $dias_hasta = $user['dias_hasta_pago'] ?? null;
                                $estado_pago_class = '';
                                $estado_pago_text = '';
                                ...
                                } elseif ($dias_hasta !== null && $dias_hasta < 0) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="payment-info">
                                            ...
                                            <?php if (isset($user['total_pagado']) && $user['total_pagado'] > 0): ?>
                                    ...
                                    <td>
                                        <?php if (!empty($user['fecha_proximo_pago'])): ?>
                                            <div class="payment-info">
                                                <span class="payment-date">
                                                    <?php echo date('d/m/Y', strtotime($user['fecha_proximo_pago'])); ?>
                                                </span>
                                                <?php if ($dias_hasta !== null): ?>
                                                    <?php if ($dias_hasta < 0): ?>
                                    ...
                                    <td>
                                        <div class="table-actions" style="gap: 0.5rem;">
                                            ...
                                            <button onclick="openRegisterPaymentModal(<?php echo $user['id']; ?>, '<?php echo htmlspecialchars(addslashes($user['nombre'])); ?>')"
